...
public class UserService implements UserDetailsService {
	...
	public User getById(Long id) {
		...
		return result.orElseThrow(() -> new NotFoundException("Usuario não encontrado"));
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		if(!result.isPresent())
			throw new UsernameNotFoundException("O usuário com email "+" não existe");
		
		User user = result.get();
		
		List<GrantedAuthority> authorities = Arrays.asList(new SimpleGrantedAuthority("ROLE_"+user.getRole().name()));
		org.springframework.security.core.userdetails.User UserSpring = new
